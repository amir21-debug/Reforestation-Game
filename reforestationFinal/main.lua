...
local seedSelectionOverlay = require("code.seedSelectionOverlay")
...
animalSpawnInterval14 = 1  -- seconds between animal appearances 
  ...
  banner1 = love.graphics.newImage("images/banner1.png")
    ...
    tree2 = love.graphics.newImage("images/tree2.png")
    tree3 = love.graphics.newImage("images/tree3.png")
    missPenalty=false;
    overWatered=0;
    respawn=0;
    redX1 = true
    ...
    dialogManagerQUICKFIX = Dialove.init({
        font = love.graphics.newFont('fonts/CCRedAlert.ttf', 16)
    })
    dialogManagerQUICKFIX:show({
        text = [[Now it's time to cut trees to collect wood. Walk over to the marked trees and press E to cut them!]],
        title = 'Zaki',
    ...
    dialogManager:push({
        text = [[Great job! Now we need to dig holes to plant our seeds. Press E over the red X's to dig your holes! {press c to close dialogue}]],
        title = 'Zaki',
        image = love.graphics.newImage('images/zaki.png')
    })
    ...
    dialogManager:push({
      text = [[Great job picking your seeds! Now it's time to plant them. Press E to turn your seeds into saplings! {press c to close dialogue} ]],
      title = 'Zaki',
      image = love.graphics.newImage('images/zaki.png')
  })

    dialogManager:push({
      text = [[Water your trees to help them grow! Press SPACE when the bar is green! {press c to close dialogue} ]],
      title = 'Zaki',
      image = love.graphics.newImage('images/zaki.png')
  })
 dialogManager:push({
        ...
        dialogManager:push({
            text = [[Quick! The fires are spreading! Extinguish them by standing next to the tree and pressing SPACE when the bar is green! {press c to close} ]],
            title = 'Zaki',
            image = love.graphics.newImage('images/zaki.png')
        })
        dialogManager:push({
            text = [[The animals have returned again! Thanks to you, the forest is alive and their home is saved!! ]],
            title = 'Zaki',
            image = love.graphics.newImage('images/zaki.png')
        })
        dialogManager:push({
        ...
        dialogManager:push({
            text = [[Now it's time to restore the forest! Select seeds to plant in the empty spaces left by cutting trees, then press SPACE when the sliding marker hits the green zone to give your sapling just the right amount of water. Nail the timing and watch it sprout into a healthy young tree-keep practicing until you master it!. {press c to close}]],
            title = 'Zaki',
            image = love.graphics.newImage('images/zaki.png')
        })
        dialogManager:push({
            text = [[Press SPACE when the sliding marker hits the green zone to water your seeds!]],
            title = 'Zaki',
            image = love.graphics.newImage('images/zaki.png')
        })
...
function love.update(dt)
    ...
    if resetStopBarTimer then
        resetStopBarTimer = resetStopBarTimer - dt
        if resetStopBarTimer <= 0 then
            stopBar = false
            movingBarX = 40
            resetStopBarTimer = nil
        end
    ...
    if gamestate==7 or gamestate==8 or gamestate == 9 or gamestate == 10 or gamestate == 11 or gamestate == 12 or gamestate == 13 or gamestate == 14 or gamestate == 15 or gamestate == 16 then
    ...
    if gamestate == 6 then
        -- Check if player has moved to a new tree position
        local currentOnTree = onRedX2(0)
        if currentOnTree and not previousOnTree then
            -- Player just moved onto a tree
            stopBar = false
            movingBarX = 40
        end
        previousOnTree = currentOnTree
        
        moveBar(dt)
    end

    --watering task bar moving thing
    if gamestate==6 or gamestate==11 or gamestate==13 then
...
if gamestate == 12 then
    if not savingAnimalsStarted then
        ...
        if savingAnimalsDelayTimer >= savingAnimalsDelayMax then
            ...
            savingAnimalsTimer = savingAnimalsMaxTime -- Start countdown for animal movement
        end
    end
    if savingAnimalsStarted and not savingAnimalsTimeUp then
        ...
        if savingAnimalsTimer <= 0 then
            ...
            savingAnimalsTimer = 0
            animalsFleeing = true
            -- all unsaved animals to flee randomly
            for _, animal in ipairs(animalsStage3) do
...
if fireExtinguishingStarted then
    ...
    if not greenZoneOffset then
        greenZoneOffset = 0
        greenZoneDirection = 1
    end
    
    -- Move the green zone back and forth
    greenZoneOffset = greenZoneOffset + (greenZoneDirection * 50 * dt)
    if greenZoneOffset > 100 then
        greenZoneDirection = -1
    elseif greenZoneOffset < 0 then
        greenZoneDirection = 1
    end
    ...
    elseif gamestate == 15 then
        -- Update logging stage
        loggingStage.update(dt)
        
        -- Show a one-time dialog when entering the tree-cutting stage
        if popOnce==0 then
            dialogManager:show({
                text = [[Only cut the marked trees! Click each marked tree to proceed.]],
                title = 'Zaki',
                image = love.graphics.newImage('images/zaki.png')
            })
            popOnce = 1
        ...
        if dialogManagerQUICKFIX then
            dialogManagerQUICKFIX:update(dt)
        end
    elseif gamestate == 16 then
        -- Update logging stage for tree-cutting replay
        loggingStage.update(dt)
    elseif gamestate == 17 then
        -- Show a one-time dialog when entering the watering mini-game
        if popOnce == 0 then
            dialogManager:show({
                text = [[Great work reaching the watering challenge! Press SPACE when the sliding marker hits the green zone to give your sapling just the right amount of water. Nail the timing and watch it sprout into a healthy young tree-keep practicing until you master it!]],
                title = 'Zaki',
                image = love.graphics.newImage('images/zaki.png')
            })
            popOnce = 1
        ...
        -- Update mini-game
        loggingStage.updateMinigame(dt)
    end
end
...
function love.draw()
    ...
    if gamestate == 0 then
        ...
        love.graphics.draw(banner1, 300, 250, 0, 3, 3)
        love.graphics.draw(banner1, 300, 325, 0, 3, 3)
        love.graphics.draw(banner1, 300, 400, 0, 3, 3)
        love.graphics.draw(banner1, 300, 475, 0, 3, 3)
        love.graphics.setColor(1, 1, 1)
        love.graphics.setFont(font1)
        love.graphics.print("FOREST", 302, 100)
        love.graphics.print("BUILDER", 290, 160)
        love.graphics.setColor(0, 0, 0)
        love.graphics.setFont(font2)
        love.graphics.print("START", 360, 264)
        love.graphics.print("STAGE 2", 346, 338)
        love.graphics.print("STAGE 3", 346, 414)
        love.graphics.print("SETTINGS", 344, 488)
        love.graphics.setColor(1, 1, 1)
    ...
    elseif gamestate == 6 then
        ...
        if totalWateredTrees==10 then
            ...
            love.graphics.print("Trees",347,280)
            love.graphics.print("Watered!",325,320)
    ...
    elseif gamestate == 6.5 then
        ...
        love.graphics.setColor(1, 1, 1)
        love.graphics.setFont(font2)
        love.graphics.print("Welcome To", 329, 175)
        love.graphics.print("Level 2!", 352, 225)
        love.graphics.setColor(0, 0, 0)
        love.graphics.setFont(font2)
        love.graphics.print("START", 360, 310)
        love.graphics.setColor(1, 1, 1)
...
elseif gamestate == 13 then
    ...
    elseif gamestate == 15 then
        ...
        -- Draw fully grown trees & animals for logging start
        -- Draw background first
        love.graphics.draw(grassBackground, 0, 0, 0, 1.7, 1.7)
        
        -- Use the loggingStage.draw function to draw trees with target indicators
        loggingStage.draw()
        ...
        -- Draw simple progress counter in a smaller font
        love.graphics.setFont(font3) -- Use the smaller font
        love.graphics.setColor(1, 1, 1)
        love.graphics.print("Cut Trees: " .. loggingStage.cutTreeCount .. " / " .. math.ceil(#loggingStage.trees / 2), 20, 20)
        

        -- Draw dialog if active
        if dialogManager and not closeDialogue then
            if gamestate15flag==0 then
                dialogManagerQUICKFIX:draw()
            else 
                dialogManager:draw()
            end
        end
    ...
    elseif gamestate == 14.5 then
        ...
        love.graphics.setFont(font2)
        love.graphics.print("Welcome To", 329, 175)
        love.graphics.print("Level 3!", 352, 225)
        love.graphics.setColor(0, 0, 0)
        love.graphics.setFont(font2)
        love.graphics.print("START", 360, 310)
        love.graphics.setColor(1, 1, 1)
    elseif gamestate == 16 then
        -- Draw the background for the tree-cutting replay scene
        love.graphics.draw(grassBackground, 0, 0, 0, 1.7, 1.7)
        
        -- Use the loggingStage module to draw trees based on their saved cut state
        loggingStage.draw()
        ...
        love.graphics.setColor(1, 1, 1)
    elseif gamestate == 17 then
        -- First draw the gamestate 16 scene in the background
        love.graphics.draw(grassBackground, 0, 0, 0, 1.7, 1.7)
        loggingStage.draw()
        
        -- Then draw the mini-game overlay
        loggingStage.drawMinigame()
    elseif gamestate == 19 then
        -- Draw the completion screen
        loggingStage.drawCompletionScreen()
    end
end
...
function love.mousepressed(x, y, button, istouch)
    ...
    if button == 1 then
        ...
        if gamestate == 0 then
            ...
            if xclick > 300 and yclick > 400 and xclick < 300 + width * 3 and yclick < 375 + height * 3 then
                gamestate = 15
                -- Reset variables needed for gamestate 15
                popOnce = 0
                closeDialogue = false
                
                -- Initialize the logging stage
                loggingStage = require("code.loggingStage")
                loggingStage.initialize()
                
               -- Wipe previous dialogue progress
                dialogManager.dialogueQueue = {}   
                dialogManager.activeDialogue = nil 
                dialogManager.currentText = ""
                dialogManager.currentIndex = 1

                -- Now push ONLY the correct dialogue for Stage 3
                dialogManager:show({
                    text = [[Now it's time to cut trees to collect wood. Walk over to the marked trees and press E to cut them!]],
                    title = 'Zaki',
                    image = love.graphics.newImage('images/zaki.png')
                ...
                dialogManager:push({
                    text = [[Now it's time to restore the forest! Select seeds to plant in the empty spaces left by cutting trees. Then press SPACE when the sliding marker hits the green zone to give your sapling just the right amount of water. Nail the timing and watch it sprout into a healthy young tree! {press c to close}]],
                    title = 'Zaki',
                    image = love.graphics.newImage('images/zaki.png')
                })
            end
        ...
        elseif gamestate == 14.5 then
            if xclick > 300 and yclick > 300 and xclick < 300 + width * 3 and yclick < 300 + height * 3 then
                popOnce = 0  -- Reset popOnce to show dialog in gamestate 15
                closeDialogue = false  -- Ensure dialog will be shown
                
                -- Initialize the logging stage
                loggingStage = require("code.loggingStage")
                loggingStage.initialize()
                
                gamestate = 15  -- Go to Stage 3 (tree cutting) instead of main menu
    ...
    -- Handle seed selection overlay clicks in logging stage
    if gamestate == 15 and loggingStage and loggingStage.showSeedSelection then
        local handled = seedSelectionOverlay.mousepressed(x, y, button, istouch)
        if handled then return end
    end
    
    -- Handle clicks in the completion screen
    if gamestate == 19 and button == 1 then
        --loggingStage.handleCompletionScreenClick(x, y)
        love.event.quit()
        return
    end
end
...
function love.keypressed(key, scancode, isrepeat)
    ...
    elseif key == 'e' and gamestate == 15 then
        -- If in planting mode after seed selection
        if loggingStage and loggingStage.readyToPlant then
            -- Plant seeds in holes
            for i, hole in ipairs(seedPlantingP2.holePositions) do
                if not seedPlantingP2.plantedHoles[i] then
                    local holeX = math.floor(hole.x / tilewidth)
                    local holeY = math.floor(hole.y / tileheight)
                    
                    if spritePlayer.tile_x == holeX and spritePlayer.tile_y == holeY then
                        -- Randomly pick a seed type from selected
                        local selectedSeeds = loggingStage.selectedSeeds
                        if #selectedSeeds > 0 then
                            local chosenSeed = selectedSeeds[math.random(#selectedSeeds)]
                            
                            local seedImg = love.graphics.newImage("images/" .. chosenSeed .. "Seed.png")
                            ...
                            -- Play planting sound if available
                            if seedClickSound then
                                seedClickSound:play()
                            end
                            
                            -- Check if all holes are planted
                            local allPlanted = true
                            for j, planted in ipairs(seedPlantingP2.plantedHoles) do
                                if not planted then
                                    allPlanted = false
                                    break
                                end
                            ...
                            if allPlanted then
                                -- All seeds planted, show message
                                if dialogManager then
                                    dialogManager:show({
                                        text = [[Excellent! You've planted all the seeds. The forest will grow back stronger than ever!]],
                                        title = 'Zaki',
                                        image = love.graphics.newImage('images/zaki.png')
                                    })
                                end
        ...
        -- If seed selection is active (shouldn't happen with e key, but just in case)
        if loggingStage and loggingStage.showSeedSelection and seedSelectionOverlay and seedSelectionOverlay.active then
            return
        end
        
        -- Normal tree cutting mode
        -- Check if player is near the target tree
        print("Cutting attempt at position: " .. spritePlayer.tile_x .. ", " .. spritePlayer.tile_y)
        if loggingStage.isPlayerNearTargetTree(spritePlayer.tile_x, spritePlayer.tile_y) then
            print("Player is near target tree! Attempting to cut...")
            -- Cut the tree
            local success = loggingStage.cutTargetTree()
            if success then
                -- Play a sound effect for cutting the tree
                if debrisSound and debrisSound:isPlaying() then
                    debrisSound:stop()
                end
                debrisSound:play()
                print("Tree cut successfully!")
            else
                print("Failed to cut tree.")
            end
        else
            print("Not close enough to the target tree.")
        end
    ...
    elseif key== "space" and gamestate==6 then
        if withinGreen() and onRedX2(0) then
            ...
            -- that we'll check in the update function
            resetStopBarTimer = 0.5  -- Reset after 0.5 seconds
   ...
   elseif key=="space" and gamestate==8 and totalDugHoles==15 then
        ...
        dialogManager:show({
            text = [[Walk over to the marked X and click "E" to dig.]],
            title = 'Zaki',
            image = love.graphics.newImage('images/zaki.png')
        })
   ...
   elseif key=="space" and gamestate==9 then
    if #seedSelectionP2.selectedSeeds > 0 then
        ...
        seedPlantingP2.spawnSeeds(seedPlantingP2.selectedSeedTypes, loggingStage)
        popOnce=0
        closeDialogue=false
        ...
        dialogManager:show({
            text = [[Great job picking your seeds! Now walk over them and play the mini-game to replant.]],
            title = 'Zaki',
            image = love.graphics.newImage('images/zaki.png')
        })
    ...
    elseif key == "space" and gamestate == 11 then
        ...
        elseif overWatered == 3 then
            ...
            barSpeedModifier=200
            for i=1,15 do
                seedPlantingP2.plantedSeeds[i].state = "covered";
            end
...
elseif gamestate == 12 and showAnimalSummary and key == "space" then
    ...
    movingBarX = 40
    barDirection = 300  -- Set to consistent speed value
    stopBar = false
    
    -- Initialize random green zone positions for each burning tree
    fireGreenZonePositions = {}
    for i = 1, #burningTrees do
        -- Generate a random position along the bar (40 to 680)
        local randomPos = math.random(40, 680)
        table.insert(fireGreenZonePositions, randomPos)
    end
...
elseif gamestate == 13 then
    if key == "space" then
        -- Check if player is near a burning tree
        local nearBurningTree = false
        for i, tree in ipairs(burningTrees) do
            local treeX = math.floor(tree.position.x / tilewidth)
            local treeY = math.floor(tree.position.y / tileheight)
            ...
            if spritePlayer.tile_x == treeX and spritePlayer.tile_y == treeY and tree.active then
                ...
                if withinGreen() then
                    ...
                    -- Mark this tree as extinguished
                    tree.extinguished = true
                    
                    -- Check if all fires are extinguished
                    local allExtinguished = true
                    for j, checkTree in ipairs(burningTrees) do
                        if checkTree.active then
                            allExtinguished = false
                            break
                        end
                    end
                    
                    if allExtinguished then
        ...
        -- If all fires are extinguished and player presses space (not near a tree)
        if fireExtinguishingComplete and not nearBurningTree then
            ...
            -- Stop fire sound if it's still playing
            if fireSound and fireSound:isPlaying() then
                fireSound:stop()
            end
            
            -- Clear previous dialogue and show completion message
            dialogManager.dialogueQueue = {}
            dialogManager.activeDialogue = nil
            
            dialogManager:show({
                text = [[The animals are returning to the forest now that the fires are gone! Your quick action saved their home.]],
                title = 'Zaki',
                image = love.graphics.newImage('images/zaki.png')
            })
    ...
    elseif key == "left" or key == "right" or key == "up" or key == "down" then
        -- Reset the bar when player moves to a new position
        stopBar = false
        movingBarX = 40
    end
    -- In the love.keypressed function, add a condition to handle transitioning from state 13 to 14
elseif key == "space" and gamestate == 14 then
    gamestate = 14.5
...
elseif key == "space" and gamestate == 15 then
    -- If seed selection overlay is active and complete
    if loggingStage and loggingStage.showSeedSelection and seedSelectionOverlay and seedSelectionOverlay.active then
        if seedSelectionOverlay.keypressed(key) then
            -- Seed selection is complete, transition to gamestate 16 (tree-cutting replay)
            loggingStage.finishSeedSelection()
            
            -- Switch to gamestate 16 for tree-cutting replay
            gamestate = 16
            popOnce = 0
    ...
    -- Check if all trees have been cut and we need to continue to the final phase
    if loggingStage and loggingStage.continueToFinalPhase() then
        -- Play a transition sound if available
        if debrisSound and debrisSound:isPlaying() then
            debrisSound:stop()
        end
        debrisSound:play()
        
        -- Show dialog explaining seed selection
        
            dialogManager.dialogueQueue = {}
            dialogManager.activeDialogue = nil
            dialogManager:show({
                text = [[Now it's time to restore the forest! Select seeds to plant in the empty spaces left by cutting trees. {press c to close}]],
                title = 'Zaki',
                image = love.graphics.newImage('images/zaki.png')
            })
...
elseif key == "space" and gamestate == 16 then
    -- Switch back to gamestate 15 for planting phase
   -- popOnce = 0  -- Reset popOnce to show dialog in gamestate 15
  --  closeDialogue = false  -- Ensure dialog will be shown
    -- gamestate = 15
elseif key == "space" and gamestate == 17 then
    -- Handle mini-game input
    local success = loggingStage.handleMinigameInput()
    
    -- Only return to gamestate 16 after success
    if success then
        gamestate = 16
    end
    -- If unsuccessful, the mini-game will reset itself and stay in gamestate 17
end
...
function drawWateringMinigameP2()
    -- Only draw if we're in gamestate 13
    if gamestate ~= 13 then return end
    
    -- Draw the fire extinguishing minigame
    love.graphics.setColor(1,.1,.2)
    -- Background of watering task bar
    love.graphics.rectangle("fill", 40, 500, 720, 75)
    
    -- For fire extinguishing, use a random green zone position for each tree
    -- Initialize tree-specific green zone positions if not already done
    if not fireGreenZonePositions then
        fireGreenZonePositions = {}
        for i = 1, #burningTrees do
            -- Generate a random position along the bar (40 to 680)
            local randomPos = math.random(40, 680)
    ...
    -- Find which burning tree the player is on
    local treeIndex = 0
    for i, tree in ipairs(burningTrees) do
        local treeX = math.floor(tree.position.x / tilewidth)
        local treeY = math.floor(tree.position.y / tileheight)
    ...
    -- Draw green zone at the position for this tree
    if treeIndex > 0 and fireGreenZonePositions[treeIndex] then
        love.graphics.setColor(0,.9,.4)
        love.graphics.rectangle("fill", fireGreenZonePositions[treeIndex], 500, 80, 75)
    else
        -- Default position if no tree is found
        love.graphics.setColor(0,.9,.4)
        love.graphics.rectangle("fill", 680, 500, 80, 75)
    end
...
function withinGreen()
    -- If the bar is within the green zone on the watering mini game return true
    if gamestate == 13 then
        -- For fire extinguishing, check against the tree-specific green zone
        if not fireGreenZonePositions then
            return false
        end
        ...
        -- Check if bar is in the green zone for this tree
        if treeIndex > 0 and fireGreenZonePositions[treeIndex] then
            local greenZoneX = fireGreenZonePositions[treeIndex]
            if movingBarX >= greenZoneX and movingBarX < (greenZoneX + 80) then
                return true
            end
        end
        
        return false
    elseif gamestate == 6 or gamestate == 11 then
        -- Original watering minigame logic for other gamestates
        -- Find which tree position the player is on
        local treeIndex = 0
        for i = 1, 15 do
            if isPlayerOnTreePosition(i) then
                treeIndex = i
                break
            end
        end
        
        -- Get the green zone position for this tree
        local greenZoneX = 695 -- default position
        if treeIndex > 0 and treeGreenZonePositions[treeIndex] then
            greenZoneX = treeGreenZonePositions[treeIndex]
        end
        
        if movingBarX >= greenZoneX and movingBarX < (greenZoneX + 50) then
            return true
        else
            return false
        end
...
totalRemovedDebris=0
    ...
    -- Initialize random green zone positions for each tree
    treeGreenZonePositions = {}
    for i = 1, 15 do
        -- Generate a random position along the bar (40 to 710)
        local randomPos = math.random(40, 710)
        table.insert(treeGreenZonePositions, randomPos)
    end
end

function moveBar(dt)
    -- Only apply speed modifier in non-fire extinguishing states
    local speed = (gamestate == 13) and 300 or barSpeedModifier
    
    if movingBarX < 40 then
        -- If we hit the left boundary, reverse direction
        barDirection = speed
        movingBarX = 41
    elseif movingBarX > 740 then
        -- If we hit the right boundary, reverse direction
        barDirection = -speed
        movingBarX = 739
    end
    
    -- Move the bar at consistent speed in either direction
    if not stopBar then
...
function drawWateringMinigameP2()
    --draw all the trees in their proper location
        ...
        if gamestate == 13 then
            ...
            --zone - find which tree position the player is on
            local treeIndex = 0
            for i = 1, 15 do
                if isPlayerOnTreePosition(i) then
                    treeIndex = i
            ...
            -- Get the green zone position for this tree
            local greenZoneX = 710 -- default position
            if treeIndex > 0 and treeGreenZonePositions[treeIndex] then
                greenZoneX = treeGreenZonePositions[treeIndex]
            end
            
            -- Draw green zone at the random position
            love.graphics.setColor(0,.9,.4)
            love.graphics.rectangle("fill", greenZoneX, 500, 50, 75)
        end
...
function gamestate6TreeDrawForefront()
    if seedSelection.selectedSeed=="cedar" then
        if redX1==false then
    ...
    elseif seedSelection.selectedSeed=="acacia" then
        if redX1==false then
            love.graphics.draw(tree2, 3, 400,0,3,3)
        end
        if redX3==false then
            love.graphics.draw(tree2, 160, 400,0,3,3)
        end
        if redX6==false then
            love.graphics.draw(tree2, 290, 400,0,3,3)
        end
        if redX8==false then
            love.graphics.draw(tree2, 455, 400,0,3,3)
        end
        if redX10==false then
            love.graphics.draw(tree2, 600, 400,0,3,3)
        end
    else
        if redX1==false then
            love.graphics.draw(tree3, -27, 430,0,3,3)
        end
        if redX3==false then
            love.graphics.draw(tree3, 130, 430,0,3,3)
        end
        if redX6==false then
            love.graphics.draw(tree3, 260, 430,0,3,3)
        end
        if redX8==false then
            love.graphics.draw(tree3, 425, 430,0,3,3)
        end
        if redX10==false then
            love.graphics.draw(tree3, 570, 430,0,3,3)
        end
    end
end
...
function gamestate6TreeDrawBackground()
    ...
    elseif seedSelection.selectedSeed=="acacia" then
        if redX2==false then
            love.graphics.draw(tree2, 3, 330,0,3,3)
        end
        if redX4==false then
            love.graphics.draw(tree2, 160, 330,0,3,3)
        end
        if redX5==false then
            love.graphics.draw(tree2, 290, 330,0,3,3)
        end
        if redX7==false then
            love.graphics.draw(tree2, 455, 330,0,3,3)
        end
        if redX9==false then
            love.graphics.draw(tree2, 600, 330,0,3,3)
        end
    else
        if redX2==false then
            love.graphics.draw(tree3, -27, 360,0,3,3)
        end
        if redX4==false then
            love.graphics.draw(tree3, 130, 360,0,3,3)
        end
        if redX5==false then
            love.graphics.draw(tree3, 260, 360,0,3,3)
        end
        if redX7==false then
            love.graphics.draw(tree3, 425, 360,0,3,3)
        end
        if redX9==false then
            love.graphics.draw(tree3, 570, 360,0,3,3)
        end
    end
...
-- After the initial variable declarations, around line 20-30
-- Random green zone positions for each tree hole
treeGreenZonePositions = {}

-- Add this function before or after withinGreen function
-- Helper function to determine which tree position the player is on
function isPlayerOnTreePosition(index)
    if index == 1 and spritePlayer.tile_x == 2 and spritePlayer.tile_y == 2 then
        return true
    elseif index == 2 and spritePlayer.tile_x == 6 and spritePlayer.tile_y == 2 then
        return true
    elseif index == 3 and spritePlayer.tile_x == 10 and spritePlayer.tile_y == 2 then
        return true
    elseif index == 4 and spritePlayer.tile_x == 14 and spritePlayer.tile_y == 2 then
        return true
    elseif index == 5 and spritePlayer.tile_x == 18 and spritePlayer.tile_y == 2 then
        return true
    elseif index == 6 and spritePlayer.tile_x == 2 and spritePlayer.tile_y == 6 then
        return true
    elseif index == 7 and spritePlayer.tile_x == 6 and spritePlayer.tile_y == 6 then
        return true
    elseif index == 8 and spritePlayer.tile_x == 10 and spritePlayer.tile_y == 6 then
        return true
    elseif index == 9 and spritePlayer.tile_x == 14 and spritePlayer.tile_y == 6 then
        return true
    elseif index == 10 and spritePlayer.tile_x == 18 and spritePlayer.tile_y == 6 then
        return true
    elseif index == 11 and spritePlayer.tile_x == 2 and spritePlayer.tile_y == 10 then
        return true
    elseif index == 12 and spritePlayer.tile_x == 6 and spritePlayer.tile_y == 10 then
        return true
    elseif index == 13 and spritePlayer.tile_x == 10 and spritePlayer.tile_y == 10 then
        return true
    elseif index == 14 and spritePlayer.tile_x == 14 and spritePlayer.tile_y == 10 then
        return true
    elseif index == 15 and spritePlayer.tile_x == 18 and spritePlayer.tile_y == 10 then
        return true
    end
    return false
end
